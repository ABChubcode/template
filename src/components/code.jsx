...
/**
  ...
  const handleGetAllClaimableReward = async () => {
    const result = await ffContract.methods.getAllClaimableReward(investor).call({
      from: address
    });
  ...
  const handleGetDepositState = async () => {
    ...
    setDepositState({
      investor: depositState.investor,
  ...
  const handleGetOwnedDeposits = async () => {
    const result = await ffContract.methods.getOwnedDeposits(investor).call({
      from: address
    });
  ...
  const handleGetTotalAmount = async () => {
      ...
      const totalClaimableRewardInWei = await ffContract.methods.getAllClaimableReward(investor).call();
...
const handleCalculateAmount = async () => {
    ...
    const depositIDs = await ffContract.methods
        .ownedDeposits(investor, { from: investor })
        .call();
    ...
    const totalClaimableRewardInWei = await ffContract.methods
      .getAllClaimableReward(investor)
      .call();
        ...
        <div className="flex md:flex-row flex-col items-start justify-between md:p-20 py-12 px-4">
          <div className="flex flex-1 justify-start items-start flex-col md:mr-10">
            ...
            <div>
                ...
                <input className="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism"
                      type="text"
                      value={investor}
          ...
          <input className="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism"
            ...
            <div className="p-3 flex justify-end items-start flex-col rounded-xl h-40 sm:w-72 w-full my-5 eth-card white-glassmorphism ">
              <div className="flex justify-between flex-col w-full h-full">
                ...
                <div>
                  <p className="text-white font-light text-sm min-h-[50px] sm:px-0 px-1 sm:min-w-[100px]">
                    {address}
                  </p>
                  <p>    
                    {claimableReward && <p className="text-white font-semibold text-lg mt-1">Claimable Reward: {claimableReward}</p>}
                  </p>
            ...
            <div className="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism">
              ...
              {withdrawCapitalResult && (
                ...
                {depositState && (
                  <div>
                    <p className="text-white font-semibold text-lg mt-1">Investor: {depositState.investor}</p>
